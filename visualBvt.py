#Code for automation:
#this is a multi user, event driven automated BVT for a high definition videoconferencing application 
#all functions def() are visual test patterns
...
config.read('C:\\SikuliTest\\UserB.ini')
...
f = open(WebPath + SmokeTest,'w')
#f.write('SmokeTest Build' + Number + '<br />')
f.write('<font color= "#347C17">B_configuration file read successfully</font><br />')
f.close()
...
ContactList_TOF13CL = config.getint('ContactList','TOF5CL')
ContactList_TOF14CL = config.getint('ContactList','TOF6CL')
...
def SignOut():
    ...
    global WebPath
    wait(5)
    if exists(ContactList_DropList, 10):
        ...
        f.write('<font color="#347C17">B_SignOut sign out</font><br />')
        f.close()
    wait(10)
#    if exists(LoginPanel,FOREVER):
#        f = open(WebPath + SmokeTest,'a')
#        f.write('<font color="#347C17">B_fail to exit</font><br />')
#        f.close()
#        wait(10)
#        SignOut()
...
def RemoveContact():
    ...
    if exists(ContactList_GreenDot):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_RemoveContact found contact to remove</font><br />')
        f.close()
        wait(5)
        rightClick(ContactList_GreenDot)
        type(Key.DOWN + Key.ENTER)
        if exists(ContactList_MessageVerifyRemoval, 10):
            click(Pattern(ContactList_MessageVerifyRemoval).targetOffset(TOF13CL,TOF14CL))
            f = open(WebPath + SmokeTest,'a')

            f.write('<font color="#347C17">B_RemoveContact contact removed</font><br />')
            f.close()
...
def LeaveConference():
    ...
    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#347C17">B_LeaveConference left conference</font><br />')
    f.close()
...
def UserADrawsLens():
    ...
    if exists(Conference_ControlPanel,FOREVER ):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_UserADrawsLens control panel present</font><br />')
        f.close()
        wait(10)
        click(Conference_DrawLens)
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_UserADrawsLens drawing lens</font><br />')
        f.close()
        setShowActions(True)
        ...
        while run.getX() < end.getX():
            run = run.right(stepX).below(stepY) # use below
instead of down a
        mouseMove(run)
        mouseMove(end)
        mouseUp()
        wait(5)
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_UserADrawsLens lens drawn</font><br />')
        f.close()
        wait (10)
        click (Conference_StopSharing)
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_UserADrawsLens stop sharing the lens</font><br />')
        f.close()
...
def ConferenceInviteAtoB():
    ...
    if exists(Conference_GreenDot, FOREVER): # verifying confernce invite is sent only when recipient is ready
        ...
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_ConferenceInviteAtoB enable user for invite</font><br />')
        f.close()
    if exists(Conference_BlueLaunchWindow, 10): # check for the color change
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_ConferenceInviteAtoB user ready for invite</font><br />')
        f.close()
        wait(10)
        click(Conference_Start)
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_ConferenceInviteAtoB invite sent</font><br />')
        f.close()
    if exists(Conference_InConferenceSpeaker, FOREVER):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_ConferenceInviteAtoB meeting request accepted</font><br />')
...
def AllLeftConference():
    ...
    global SmokeTest
    global WebPath
    if exists(Conference_EndedAllLeft,FOREVER):
        wait(5)
        click(Pattern(Conference_EndedAllLeft).targetOffset(TOF7CL,TOF8CL))
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C2C">B_all left the conference</font><br />')
        f.close()
    else:
        pass


def VerifyLensShared():
    setBundlePath (BundlePathCF)
    global SmokeTest
    global WebPath
    if exists(Conference_ViewingLens,FOREVER):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C2C">B_viewing the shared desktop</font><br />')
        f.close()


def AcceptConferenceInviteFromA():
    setBundlePath (BundlePathCF)
    global SmokeTest
    global WebPath
    if exists(Conference_AcceptMeetingRequest,FOREVER):
        click(Pattern(Conference_AcceptMeetingRequest).targetOffset(TOF5CL,TOF6CL)) #accept invite
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C2C">B_conference invite accepted</font><br />')
        f.close()
...
def AcceptAddtoList():
    ...
    global WebPath
    if exists(ContactList_AcceptAddToList,FOREVER):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C2C">B_accept add to list shown</font><br />')
        f.close()
        click(Pattern(ContactList_AcceptAddToList).targetOffset(89,40))
    else:
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C2C">B_add dialog not received</font><br />')
        f.close()
    if exists(ContactList_GreenDot, 60):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C2C">B_contact added</font><br />')
        f.close()
#        AcceptConferenceInvitefromA()
#    else:
#        f = open(WebPath + SmokeTest,'a')
#        f.write('<font color="#347C2C">B_contact not added</font><br />')
#        f.close()
#        AcceptAddtoList()
...
def AddContact_AtoB():
    ...
    if exists(ContactList_WelcomePanel, 20):
        wait(20)
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_AddContact_AtoB begin add user</font><br />')
        f.close()
    ...
    if exists(ContactList_AddContact, 20):
        ...
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_AddContact_AtoB add contact dialog displayed</font><br />')
        f.close()
        type(ContactList_AddContactUser)
        wait(10)
        click(Pattern(ContactList_AddContact).targetOffset(ContactList_TOF1CL,ContactList_TOF2CL))
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_AddContact_AtoB verified user id exists</font><br />')
        f.close()
        wait(10)
    if exists(ContactList_GreenDot, 60):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_AddContact_AtoB add contact successful</font><br />')
        f.close()
        wait(10)
...
def MediaRouter():
    ...
    if exists(SignIn_MediaRouter, 30):
        ...
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_MediaRouter media router droplist found</font><br />')
        f.close()
        wait(1)#wait needed to support droplist navigation
        type(Key.DOWN + Key.DOWN + Key.DOWN + Key.DOWN + Key.ENTER)# selects Fort Collins
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_MediaRouter Fort Collins selected</font><br />')
        f.close()
        wait(3)
        ...
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_MediaRouter media router submitted</font><br />')
        f.close()
        wait(5)
    else:
        pass
...
def LoopSignIn():
    ...
    global WebPath
    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#0000FF">B_LoopSignIn Login user A is ' + LoginUser + '</font><br />')
    f.close()
    if exists(SignIn_EULA, 5):
        click(Pattern(SignIn_EULA))
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#FF0000">B_LoopSignIn ERROR EULA shown after user accepted during initial sign in ERROR</font><br />')
        f.close()
    if exists (ContactList_WelcomePanel, 30):
        wait(1)
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#0000FF">B_LoopSignIn user automatically signed in</font><br />')
        f.write('<font color="#0000FF">B_LoopSignIn welcome panel detected</font><br />')
        f.close()
...
def FirstSignIn():
    ...
    global WebPath
    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#347C17">B_FirstSignIn Login user A is ' + LoginUser + '</font><br />')
    f.close()
    if exists (FullLoginPanel, 30):
        wait(1)
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_FirstSignIn login panel detected</font><br />')
        f.close()
        type(Pattern(SignIn_SignIn).targetOffset(TOF1S,TOF2S), LoginUser)
        wait(1)
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_FirstSignIn login user entered</font><br />')
        f.close()
        type(Pattern(PasswordInputField).targetOffset(TOF1L,TOF2L), Password)
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_FirstSignIn password entered</font><br />')
        f.close()
        wait(1)
        click (Pattern(SignIn_OKButton))

        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_FirstSignIn credentials submitted</font><br />')
        f.close()
        wait(1)
    if exists(SignIn_EULA, 30):
        click(Pattern(SignIn_EULA))
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_FirstSignIn EULA accepted</font><br />')#add handler for if the image does not exist
        f.close
...
def Uninstall():
    ...
    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#347C17">B_Uninstall uninstalling build' + Number +'</font><br />')
    f.close()
    if not exists(Launch, 60):
        wait(1)
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_Uninstall successfuly uninstalled build' + Number +'</font><br />')
        f.close()
...
def Install():
    ...
    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#347C17">B_Install installing build' + Number +'</font><br />')
    f.close()
    while not exists(Launch, FOREVER):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_Install successfully installed build' + Number+ '</font><br />')
        f.close()
...
def ExitVCDKillProcess():
    ...
    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#347C17">B_ExitVCDKillProcess system tray expanded</font><br />')
    f.close()
    if exists(Desktop_SysTraySkyRoomIcon, 10):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_ExitVCDKillProcess click on the VCD icon</font><br />')
        f.close()
        ...
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_ExitVCDKillProcess exit selected from the VCD menu</font><br />')
        f.close()
        ...
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_ExitVCDKillProcess VCD exited</font><br />')
        f.close()
        wait(2)
    else:
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_ExitVCDKillProcess
VCD icon not found</font><br />')
        f.close()
...
#    if exists(Launch,10):
#        f = open(WebPath + SmokeTest,'a')
#        f.write('<font color="#347C17">B_ExitVCDProcess function launched</font><br />')
#        f.close()
#    else:
#        f = open(WebPath + SmokeTest,'a')
#        f.write('<font color="#347C17">B_desktop VCD icon not found, begin install</font><br />')
#        f.close()
...
def LaunchApp():
    ...
    if exists(Launch, 10):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_LaunchApp found VCD icon</font><br />')
        f.close()
        doubleClick(Pattern(Launch))
    else:
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#FF000">B_LaunchApp find VCD icon failed</font><br />')
        f.write('<font color="#347C17">B_LaunchApp retry finder VCD icon</font><br />')
        f.close()
        LaunchApp()
    if exists(FullLoginPanel, 10): # Looking for the login screen
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#347C17">B_LaunchApp launches VCD</font><br />')
        f.close()
...
def LoopSignInConferenceExit():
    ...
    while (count < 2):
        ...
        f = open(WebPath + SmokeTest,'a')
        f.write('B_LaunchApp iteration ' + (str (count)) + '<br />')
        f.close()
        LaunchApp()

        f = open(WebPath + SmokeTest,'a')
        f.write('B_LoopSignIn iteration ' + (str (count)) + '<br />')
        f.close()
        LoopSignIn()

        f = open(WebPath + SmokeTest,'a')
        f.write('B_AcceptAddToList iteration ' + (str (count)) + '<br />')
        f.close()
        AcceptAddtoList()#needs synch

        f = open(WebPath + SmokeTest,'a')
        f.write('B_AcceptConferenceInviteFromA iteration ' + (str (count)) + '<br />')
        f.close()
        AcceptConferenceInviteFromA()#needs synch

        f = open(WebPath + SmokeTest,'a')
        f.write('B_UserADrawsLens iteration ' + (str (count)) + '<br />')
        f.close()
        VerifyLensShared()

        f = open(WebPath + SmokeTest,'a')
        f.write('B_AllLeftConference iteration ' + (str (count)) + '<br />')
        f.close()

        AllLeftConference()

        f = open(WebPath + SmokeTest,'a')
        f.write('B_RemoveContact iteration ' + (str (count)) + '<br />')
        f.close()
        RemoveContact()

        f = open(WebPath + SmokeTest,'a')
        f.write('B_SignOut iteration ' + (str (count)) + '<br />')
        f.close()
        SignOut()
        count = count + 1
    f = open(WebPath + SmokeTest,'a')
    f.write('B_ Smoke test Complete' + (str (count)) + '<br />')
    f.close()
    exit()
...
def FirstPass():
    ...
    global count
    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#000000">B_Firstpass executing</font><br />')
    f.close()
    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#000000">B_LaunchApp executing </font><br />')
    f.close()
    LaunchApp()

    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#000000">B_FirstSignIn executing </font><br />')
    f.close()
    FirstSignIn()

    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#000000">B_Media Router executing</font><br />')
    f.close()
    MediaRouter()


    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#000000">B_Exit executing </font><br />')
    f.close()
    SignOut()
...
def MasterInstall():
    ...
    setBundlePath(BundlePathSignIn)
    f = open(WebPath + SmokeTest,'a')
    f.write('<font color="#000000">B_MasterInstall executing</font><br />')
    f.close()
    if exists(Launch,10):
        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#000000">B_ExitVCDKillProcess executing</font><br />')
        f.close()
        ExitVCDKillProcess()

        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#000000">B_Uninstall executing</font><br />')
        f.close()
        Uninstall()

        f = open(WebPath + SmokeTest,'a')
        f.write('<font color="#000000">B_Install executing</font><br />')
        f.close()
        Install()
    else:
        f = open(WebPath + SmokeTest,'a')
